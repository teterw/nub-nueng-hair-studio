      ...
      <div className="mt-7 grid gap-10 sm:grid-cols-2 sm:gap-12">
            ...
            {shop.hours.days.map((row) => (
              <div key={row.day} className="border-b border-line py-2.5 last:border-b-0">
                <div className="flex items-baseline justify-between gap-4">
                  ...
                  <dd className={row.highlight ? "font-semibold" : "text-ink/75"}>{row.hours}</dd>
                </div>
                {row.note ? <p className="mt-0.5 text-sm text-accent">{row.note}</p> : null}
              </div>
            ))}
          </dl>
        ...
        <div>
          ...
          <a
            ...
            className="mt-3 inline-block border-b border-rule pb-0.5 text-sm hover:border-ink"
          >
            เปิดใน Google Maps
          </a>
        </div>
